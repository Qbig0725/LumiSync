...
type UserData = { name: string; lumens: number; brightness: number; chronotype: Chronotype; appTheme: AppTheme; ownedThemes: AppTheme[]; tasks: LightTask[]; weeklyTasks: WeeklySchedule; settings: SettingsData; rewardClaims: RewardClaims; dailyMissionClaims: Record<string, string[]>; firstSeen: string; missionClaimedDate?: string }
...
const defaultData: UserData = { name: '', lumens: 1250, brightness: 68, chronotype: 'Intermediate', appTheme: 'lunar', ownedThemes: ['lunar'], tasks: initialTasks, weeklyTasks: createWeeklySchedule(), settings: defaultSettings, rewardClaims: {}, dailyMissionClaims: {}, firstSeen: '' }
const tabs = [
...
function readCookie(): UserData | null {
  const hydrate = (raw: string) => {
    ...
    return { ...defaultData, ...saved, tasks: legacyTasks, weeklyTasks: saved.weeklyTasks ?? createWeeklySchedule(legacyTasks), settings: { ...defaultSettings, ...saved.settings }, rewardClaims: saved.rewardClaims ?? {}, dailyMissionClaims: saved.dailyMissionClaims ?? {}, appTheme, ownedThemes: Array.from(new Set(['lunar', ...ownedThemes])) }
  }
  try {
...
export function LumiSyncApp() {
  ...
  const todayMissionKey = new Date().toLocaleDateString('en-CA')
